...
`use_figma` を以下の指針で呼び出す（詳細 API は `figjam-api.md` を参照）:
...
- [`figjam-api.md`](./figjam-api.md): FigJam で使える Plugin API のサンプル
- [`prompting.md`](./prompting.md): ヒアリング用の質問テンプレとスタイル判定のヒント
